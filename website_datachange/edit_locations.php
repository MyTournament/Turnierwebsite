...
if($successfulLogin == 1){
    ...
    if($action == 'new_rating'){
        ...
        myDb_execute($conn, $TurnierID, $bn, $sql, array($name, $description, $sterne, $fk_location, $accountId));

        //STATISTIK
        $typeId = 2;
        $add_text = "für $location_name: $sterne &#9733;";
        $sql = "INSERT INTO Pausenraum_Achievement (fk_account, fk_type, add_text) VALUES (?, ?, ?)";
        myDb_execute($conn, $TurnierID, $bn, $sql, array($accountId, $typeId, $add_text));
    }
