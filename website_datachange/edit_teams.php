...
//if(strlen($Teamname)>2 && strlen($Spieler1)>2){
    ...
    if (isset($_POST['cb_action']) && $_POST['cb_action'] === 'check') {
        ...
        $statusMessage = $res['ok']
            ? 'Captcha best�tigt. Du kannst jetzt absenden.'
    ...
    if($action == 'Anmelden'){
        ...
        require_once __DIR__ . '/../website_functionalities/captcha_blanki.php';
        // Nur Absenden erlauben, wenn vorher �ber den Captcha-Button best�tigt wurde
        if (!CaptchaBlanki::passed('register')){
            if (session_status() !== PHP_SESSION_ACTIVE) { @session_start(); }
            $_SESSION['flash_error_register'] = 'Bitte zuerst das Captcha best�tigen.';
            $_SESSION['captcha_attempted_register'] = 1;
        ...
        */
        ...
        if($captchaOk){
			...
			if($turnier_phase_ID==12){ // Falls Warteliste
				$message2 = "Leider sind die Plaetze des Turniers vorl�ufig voll. Dein Team wurde der Warteliste hinzugef�gt und kann eventuell noch nachr�cken. Falls Plaetze frei werden, sagen wir euch Bescheid. \r\n \r\n";
			}else{
			...
			echo "<script>console.log('Step: WEITERLEITUNG ZUR�CK - mit eventueller TestTurnierID')</script>";
		...
		if($action == 'Abmelden'){
			if($teamId == "auffangbeckenfueralledienichtcheckendassmanhierwasausw�hlenmuss"){
				$message = "Du musst schon ein Team ausw�hlen du Keck";
