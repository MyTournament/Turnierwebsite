...
/*function get_rights_of_user($conn, $TurnierID, $bn, $pw, $begegnungId){
  ...
  $stmt = $conn->prepare("SELECT * FROM `Turnier_Team` WHERE geloescht = 0 ORDER BY ID");
  $stmt->execute();
  $teamListe = $stmt->get_result();

  $spielGehoertZuTeam = 0; //false
  $teamBearbeitungsrecht = 0;

  foreach ($teamListe as $t){
    if(
      $t['kuerzel'] == $bn and
      $t['password'] == $pw
    ){
      $successfulLogin = 1;
      $teamBearbeitungsrecht = $t["bearbeitungsrechte"];

      $stmt = $conn->prepare("SELECT * FROM Turnier_Team WHERE geloescht = 0 AND fk_turnier = ? ORDER BY ID");
      $smtp = "SELECT * FROM Turnier_Team, Turnier_Begegnung WHERE Turnier_Team.geloescht = 0 AND Turnier_Begegnung.id = ? AND ((Turnier_Begegnung.fk_heimteam IN (SELECT id FROM Turnier_Team WHERE geloescht = 0 AND kuerzel = ? AND `password` = ?)) OR (Turnier_Begegnung.fk_auswaertsteam IN (SELECT id FROM Turnier_Team WHERE geloescht = 0 AND kuerzel = ? AND `password` = ?)))"; // AND fk_turnier = '$TurnierID'
